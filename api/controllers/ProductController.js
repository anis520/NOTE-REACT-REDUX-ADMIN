...
const {name,regular_price,stock,photo,catagorys,tags,brand}=req.body
...
if(req.files.gallary){
 ...
 try {
  ...
  const product =await Product.create({name,slug,stock:Number(stock),catagorys:cat,tags:tag,brand  ,photo:req.files.photo[0].filename,gallary:gal,regular_price})
